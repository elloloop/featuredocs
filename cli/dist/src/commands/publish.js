...
export async function publishCommand(options) {
    ...
    if (!fs.existsSync(featuresPath) || !fs.existsSync(productPath)) {
        console.log(chalk.red("docs/ not found. Run 'featuredocs init' first."));
        process.exit(1);
    }
    const { product, version } = options;
    const status = options.live ? "published" : "draft";
    const videosDir = options.videosDir ?? path.join(process.cwd(), "demos", "videos", "tablet_trimmed");
    ...
    if (r2Config && bucket) {
        ...
        if (fs.existsSync(videosDir)) {
            ...
            const uploadedKeys = await uploadDirectoryToR2(resolvedR2Config, videosDir, videoPrefix);
            console.log(`  Uploaded ${uploadedKeys.length} video file(s)`);
        }
        else {
            console.log(chalk.yellow(`Videos directory not found: ${videosDir}. Skipping video upload.`));
        }
    }
    else {
        console.log(chalk.yellow("R2 not configured. Skipping video upload."));
    }
    ...
    if (contentDir) {
        ...
        const manifest = JSON.parse(rawFeatures);
        manifest.status = status;
        manifest.version = version;
        fs.writeFileSync(path.join(targetDir, "features.json"), JSON.stringify(manifest, null, 2) + "\n", "utf-8");
        ...
        for (const entry of entries) {
            ...
            for (const mdFile of mdFiles) {
                fs.copyFileSync(path.join(sourceLocaleDir, mdFile), path.join(targetLocaleDir, mdFile));
            }
        ...
        await uploadFileToR2(resolvedR2Config, tmpFeaturesPath, `${product}/v${version}/features.json`);
        ...
        console.log(chalk.yellow("No content destination configured. Set contentDir in .featuredocs.json or R2 credentials."));
    }
    console.log("");
    console.log(`Status: ${status === "published" ? chalk.green("published") : chalk.yellow("draft")}`);
    if (status === "draft") {
        console.log(`Run with ${chalk.cyan("--live")} to publish immediately.`);
    }
}
